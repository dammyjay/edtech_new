...
//   try {
//     // 1️⃣ Check VERIFIED users first
//     const result = await pool.query(
...
//         const schoolRow = await pool.query(
//           "SELECT id, name FROM schools WHERE created_by = $1",
//           [user.id]
//         );

//         req.session.user.school_id = schoolRow.rows[0]?.id || null;
//         req.session.user.school_name = schoolRow.rows[0]?.name || null;

//         await logActivityForUser(
//           req,
//           "School Admin logged in",
//           `School: ${req.session.user.school_name}`
//         );
...
//         req.session.user.classrooms = classroomsRes.rows || [];
...
//       if (user.role === "parent") {
//         await logActivityForUser(req, "Parent logged in", user.fullname);
//         return res.redirect("/parent/dashboard");
//       }
...
//     // ====================================
//     // ❗ USER NOT VERIFIED → CHECK PENDING
//     // ====================================
...
//       // ⚠️ Send user back to signup → OTP modal opens
//       return res.render("admin/login", {
//         error: "Account not verified. Please verify the OTP sent to your email.",
//         title: "Login",
//         redirect: redirectUrl || "",
//         pendingEmail: email, // 👈 frontend will use this
//       });
//     }
