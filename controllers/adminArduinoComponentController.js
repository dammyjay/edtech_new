...
const KNOWN_GOOD_COMPONENTS = [
  ...
  { tag: "wokwi-analog-joystick", label: "Analog Joystick", category: "Input", simulated: true },
  {
    ...
    category: "Input",
    simulated: true,
    note: "No built-in interactive control (no way to physically tilt it in the browser) — select it on the canvas to toggle its state from the selection toolbar instead.",
  },
  ...
  },
  {
    ...
    category: "Sensors",
    simulated: true,
    note: "No built-in interactive control — select it on the canvas to toggle motion detected/idle from the selection toolbar instead.",
  },
  {
    ...
    category: "Sensors",
    simulated: true,
    note: "No built-in interactive control — select it on the canvas to toggle flame detected/idle from the selection toolbar instead.",
  },
  {
    ...
    category: "Sensors",
    simulated: true,
    note: "No built-in interactive control — select it on the canvas to toggle gas detected/idle from the selection toolbar instead.",
  },
  {
    ...
    category: "Sensors",
    simulated: true,
    note: "No built-in interactive control — select it on the canvas to toggle sound detected/idle from the selection toolbar instead.",
  },
  {
  ...
  },
  {
    ...
    category: "Sensors",
    simulated: true,
    note: "No built-in interactive control — select it on the canvas to toggle pulse detected/idle from the selection toolbar instead.",
  },
  ...
  // --- Actuators --------------------------------------------------------------
  {
    ...
    category: "Actuators",
    simulated: true,
    note: "Coil on/off is real (driving COIL1/COIL2 high energizes that relay, shown as a glow) — switching what its NO/NC/P pins are electrically connected to mid-run isn't modeled, since this app's wiring is computed once when a sketch starts running.",
  },
  { tag: "wokwi-stepper-motor", label: "Stepper Motor", category: "Actuators", simulated: true },
  { tag: "wokwi-biaxial-stepper", label: "Biaxial Stepper Motor", category: "Actuators", simulated: true },
  ...
  },
  {
    ...
    category: "Modules",
    simulated: true,
    note: "Real command/pixel protocol over I2C — draws whatever a sketch actually sends (text, shapes, etc.); the SSD1306 status/read-back path isn't modeled, but nothing common needs it.",
  },
  {
    tag: "wokwi-hx711",
    ...
    category: "Modules",
    simulated: true,
    note: "No built-in interactive control — reports a fixed real 24-bit reading over the actual bit-banged protocol, not adjustable yet.",
  },
  {
    tag: "wokwi-ds1307",
    ...
    category: "Modules",
    simulated: true,
    note: "Runs a real live clock over I2C — reads back the actual current time by default, and honors rtc.adjust() to set it.",
  },
  {
    tag: "wokwi-mpu6050",
    ...
    category: "Modules",
    simulated: true,
    note: "No built-in interactive control — reports a fixed real I2C reading (level and still: accel Z = +1g, everything else zero).",
  },
  {
    ...
    category: "Modules",
    simulated: true,
    note: "Real NEC infrared protocol over the actual IRremote library — select it, then use the toolbar's \"Send NEC Code\" button to transmit a fixed address/command your sketch can decode.",
  },
  {
    tag: "wokwi-ir-remote",
    ...
    category: "Modules",
    simulated: false,
    note: "Visual only — this part has no pins in @wokwi/elements at all, so it can't be wired to anything. Use the IR Receiver above (with its \"Send NEC Code\" button) to simulate a signal instead.",
  },
  {
