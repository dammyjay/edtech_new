...
async function saveProject(manual) {
  ...
  try {
    ...
    const data = window.OfflineSync
      ? await window.OfflineSync.saveOrQueue("/labs/project/save", payload, `web:${window.currentProjectId}`)
    ...
    try {
      ...
      if (LESSON_LAB_ID) {
        ...
        if (data.labFeedback) {
          ...
          // folded into the same dialog rather than a second popup; also
          // persists in the dashboard's "Recommended for you" widget
          // either way.
